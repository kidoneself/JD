...
import com.kidoneself.jd.util.DoRequestFactory;
...
class JdApplicationTests {
    ...
    @Test
    void getUserInfo() {
        DoRequestFactory doRequestUtil = new DoRequestFactory();
        String url = "https://wq.jd.com/user_new/info/GetJDUserInfoUnion?sceneval=2";
        String response = doRequestUtil.doGet(url);
        JSONObject result = JSONObject.parseObject(response);
        JSONObject o = result.getJSONObject("data").getJSONObject("userInfo").getJSONObject("baseInfo");
        JDUser jdUser = JSON.toJavaObject(o, JDUser.class);
    }
